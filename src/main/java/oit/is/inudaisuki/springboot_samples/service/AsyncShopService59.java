...
@Service
public class AsyncShopService59 {
  boolean dbUpdated = false;

  private final Logger logger = LoggerFactory.getLogger(AsyncShopService59.class);
  ...
  public void asyncShowFruitsList(SseEmitter emitter) {
    ...
    try {
      while (true) {
        ...
        ArrayList<Fruit> fruits9 = this.syncShowFruitsList();
        emitter.send(fruits9);
        dbUpdated = false;
      }
    } catch (Exception e) {
      // 例外の名前とメッセージだけ表示する
